% Cover letter for submission to Transportmetrica B: Transport Dynamics
% (Taylor & Francis). This letter identifies the author, as intended -- it
% goes to the editor only. Transportmetrica B runs single-anonymized peer
% review (the reviewer is anonymous to the author; the author's identity is
% visible to the reviewer, via ScholarOne), so -- unlike the previous
% (Elsevier, double-blind) submission -- main.tex is not anonymized and there
% is no separate title-page file; author name and affiliation appear directly
% in the manuscript.
...
Hung Yen, Vietnam\\
\url{dqvinh87@gmail.com}

\vspace{1em}
\noindent
To the Editors,\\
\emph{Transportmetrica B: Transport Dynamics}
...
Metro Network Expansion in Asian Cities'' for consideration as a
regular research article in \emph{Transportmetrica B: Transport Dynamics}. This is a
single-anonymized submission per the journal's policy; I understand the manuscript will
be assessed for suitability by the editor and then reviewed by one independent, anonymous
expert.

The paper addresses the Metro Network Expansion Problem (MNEP), the rail-specific case of
the Transport Network Design Problem (TNDP): given an existing rail network and a limited
construction budget, decide which stations or line segments to add next. Classical TNDP
solution methods -- mixed-integer and bilevel programming, genetic algorithms, tabu
search, ant colony optimization -- buy tractability by fixing candidate corridors or line
counts by hand before optimization begins. Reformulating MNEP as a sequential decision
process solvable by reinforcement learning removes that pre-constraint; this journal has
itself recently published closely related work applying deep RL directly to transit
network design and improvement (Holliday, El-Geneidy, and Dudek, 2025), which we take as
a strong signal of fit for a methods contribution of this kind, and three further recent
papers have each taken a piece of that reformulation and shown it works: a GNN-based
spatial state representation (MetroGNN), a fairness-aware multi-objective reward
decomposed via Tchebycheff scalarization, and a tabular reinforcement-learning method that
argues deep RL's overhead is unnecessary at this problem's scale. No method combines the
first two, and none has been tested from small, data-sparse emerging metro systems to the
largest networks in the world. We propose a GNN-based, multi-objective actor-critic
policy that combines a heterogeneous spatial-graph encoder with a Tchebycheff-scalarized
reward over demand, equity, and accessibility, condition the policy itself on the sampled
objective-weight vector so one trained model spans the full efficiency-equity trade-off
surface, and add a cross-city curriculum-transfer mechanism -- pretraining on one city's
network and warm-starting another's -- that responds directly to the training-cost
critique of GNN-based methods without abandoning the GNN.
...
The main text runs to approximately 14,200 words (Introduction through Conclusion,
excluding tables, figures, and references), above the journal's typical 10,000-word
guideline; this reflects two fully reported case-study cities together with the
reproducibility and construct-validity findings described above. I would welcome the
editor's guidance on any length reduction expected before the manuscript proceeds to
review.
